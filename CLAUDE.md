...
Use `@Auto` annotation for automatic dependency injection:

```java
public class MyGame extends Game {
    @Auto
    private SceneManager sceneManager;  // Auto-injected from BlueprintRepository

    @Auto("customService")
    private CustomService service;  // Named injection

    @Scheduled(intervalMs = 1000)
    public void periodicUpdate() {
        // Called every second in executor pool
    }

    @AfterAssembly
    public void postConstruct() {
        // Called after all @Auto fields injected
    }
}
```

Components auto-register via `BlueprintAssembler.assemble(instance)` during game initialization.
...
For component-based architecture, use the `ext-ecs` extension:

```java
// Scene management
SceneManager sceneManager = new SceneManager(this);
GameScene scene = new GameScene("MainScene", this);

// GameObject creation
GameObject player = new GameObject("Player");
player.addComponent(new Sprite(...));
player.addComponent(new CustomBehavior());
player.getTransform().setPosition(100, 100);
...
**Component Lifecycle**:
- `load()` - called when component added to active scene
- `update(DeltaTime)` - called every frame
- `draw(DeltaTime, SpriteBatch)` - called during render phase
- `dispose()` - cleanup when removed

## Important Design Documents

Several `.md` files in root directory document ongoing architectural work:

- **CANVAS_ARCHITECTURE_REFACTORING.md** - Separation of Canvas (fluent facade) from CanvasRenderer (low-level operations)
- **CANVAS_FLUENT_API_DESIGN.md** - Detailed design of fluent API with `withX()` pattern and auto-execution
- **CANVAS_SKINNING_DESIGN.md** - Future skinning system for UI theming
- **CANVAS_VIEWPORT_UPDATE.md** - Viewport management and coordinate systems
- **GRADIENT_FEATURE_PLAN.md** - Gradient rendering implementation details
- **RENDERING_ORDER_GUIDE.md** - Z-ordering and layer management
- **SDF_BATCH_RENDERER_DESIGN.md** - Signed Distance Field rendering architecture

**When working on Canvas/rendering code**, reference these documents for context on design decisions.
...
## Publishing (Maven/Sonatype)

For maintainers publishing releases:

```bash
# Build and publish specific platform
...
See `PUBLISH.md` for complete release process.

## Development Branch Strategy

- **main**: Stable releases
- **devel**: Development branch (use for PRs and active development)
- Feature branches: Merge to `devel`, not `main`

Current active branch: `task/urp` (URP rendering pipeline work)
